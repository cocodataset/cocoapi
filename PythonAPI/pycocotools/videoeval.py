...
class VideoEval:
    ...
    def _prepare(self):
        ...
        for vidId in p.vidIds:
            ...
            gts += self.anns2Tracks(gt, imgIds, vidId, with_occlusion=False)
            dts += self.anns2Tracks(dt, imgIds, vidId, with_occlusion=False)
    ...
    def evaluate(self):
        ...
        maxDet = p.maxDets[-1]
        self.evalVids = [evaluateVid(vidId, catId, areaRng, maxDet)
                 for catId in catIds
    ...
    def anns2Tracks(self, anns, img_ids, vid_id, with_occlusion=True):
        ...
        for ann in anns:
            ...
            if not with_occlusion:
                if ins_id in counters.keys() and \
                        img_ind > 0 and \
...
#                 ann_by_track['iscrowd'] = ann['iscrowd']
    ...
    def evaluateVid(self, vidId, catId, aRng, maxDet):
        ...
        for g in gt:
            if g['ignore'] or (g['avgArea']<aRng[0] or g['avgArea']>aRng[1]):
                g['_ignore'] = 1
            else:
                g['_ignore'] = 0
        ...
        a = np.array([d['avgArea']<aRng[0] or d['avgArea']>aRng[1] for d in dt]).reshape((1, len(dt)))
        ...
        for k, k0 in enumerate(k_list):
            ...
            for a, a0 in enumerate(a_list):
                ...
                for m, maxDet in enumerate(m_list):
                    E = [self.evalVids[Nk + Na + i] for i in i_list]
...
class Params:
    ...
    def setDetParams(self):
        ...
        self.iouThrs = np.array([0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7])
