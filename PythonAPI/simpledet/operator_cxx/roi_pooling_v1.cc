...
inline void ROIPoolForward_v1(const Tensor<cpu, 4, Dtype> &out,
...
template<typename Dtype>
inline void ROIPoolBackwardAcc_v1(const Tensor<cpu, 4, Dtype> &in_grad,
...
template<>
Operator *CreateOp<cpu>(ROIPoolingParam_v1 param, int dtype) {
  Operator* op = NULL;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
    op = new ROIPoolingOp_v1<cpu, DType>(param);
  });
  return op;
}

Operator *ROIPoolingProp_v1::CreateOperatorEx(Context ctx, std::vector<TShape> *in_shape,
                                           std::vector<int> *in_type) const {
  DO_BIND_DISPATCH(CreateOp, param_, in_type->at(0));
}

DMLC_REGISTER_PARAMETER(ROIPoolingParam_v1);

MXNET_REGISTER_OP_PROPERTY(ROIPooling_v1, ROIPoolingProp_v1)
...
.add_arguments(ROIPoolingParam_v1::__FIELDS__());
}  // namespace op
}  // namespace mxnet
